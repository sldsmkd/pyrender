...
from pyrender.tuples import Tuple, Point, Vector
...
@then(u'a is a point')
def step_impl(context):
    assert context.a.IsPoint() == True


@then(u'a is not a vector')
def step_impl(context):
    assert context.a.IsVector() == False
...
def step_impl(context):
    assert context.a.IsPoint() == False


@then(u'a is a vector')
def step_impl(context):
    assert context.a.IsVector() == True
...
def step_impl(context):
    context.v.Normalize() == Vector(1, 0, 0)


@then(u'normalize(v) = approximately vector(0.26726, 0.53452, 0.80178)')
def step_impl(context):
    context.v.Normalize() == Vector(0.26726, 0.53452, 0.80178)
